...
class HMIDashboard {
	constructor() {
		...
		this.values = {
			...
			phDown: 75,
			pumpActive: false,
			lightsOn: false // Default to match HTML
		};
		...
		['slow', 'normal', 'fast'].forEach(speed => {
			...
			if (btn) {
				btn.addEventListener('click', () => {
					...
					this.addLogEntry(`Simulation Speed: ${speed.toUpperCase()}`);
					// Actual speed logic will be connected to backend simulation step
				});
			}
		});
	...
	initExpandableCards() {
		...
		if (luxSlider) {
			luxSlider.addEventListener('input', () => {
				this.drawLineGraph('lux-graph', this.history.lux, '#66FCF1', { maxValue: 20000 });
			});
		}
	...
	initGraphs() {
		// We'll generate dummy data initially
	}
	...
	generateInitialHistory() {
		const generateTrend = (base, variance) => {
			const arr = [];
			let val = base;
			// Generate 240 points for history scrubbing (10 days @ 1hr)
			for (let i = 0; i < 240; i++) {
				val += (Math.random() - 0.5) * variance;
				// Keep somewhat bounded
				if (val < base * 0.5) val += variance;
				if (val > base * 1.5) val -= variance;
				arr.push(Math.round(val));
			}
			return arr;
		};

		this.history.lux = generateTrend(12000, 3000); // Higher Lux Values
		this.history.humidity = generateTrend(60, 10);
		this.history.temperature = generateTrend(24, 2);

		// Fill others
		this.history.nutriA = generateTrend(80, 5);
		this.history.nutriB = generateTrend(50, 5);
		this.history.phDown = generateTrend(70, 5);
		this.history.watering = [];
	}
	...
	drawLineGraph(canvasId, data, lineColor, isStep = false) {
		const canvas = document.getElementById(canvasId);
		if (!canvas) return;
		...
		drawLineGraph(canvasId, inputData, lineColor, options = {}) {
			const canvas = document.getElementById(canvasId);
			if (!canvas) return;

			const ctx = canvas.getContext('2d');
			const width = canvas.width = canvas.parentElement.offsetWidth;
			const height = canvas.height = canvas.parentElement.offsetHeight; // Fill container (300px for Lux)

			ctx.clearRect(0, 0, width, height);

			// Handle Slider Scrubbing Logic
			let data = [...inputData];
			const slider = document.getElementById('lux-slider');
			if (canvasId === 'lux-graph' && slider) {
				const pct = parseInt(slider.value);
				// "Squeeze/Extend/Scroll": Map slider 0-100 to window position
				// If value is 100, show [End - 24] to [End]
				// If value is 0, show [0] to [24]
				if (data.length > 24) {
					const maxIndex = data.length - 24;
					const start = Math.floor((pct / 100) * maxIndex);
			...
			const maxVal = options.maxValue || Math.max(...data, 100); // Allow manual override
			const minVal = 0;
			...
			// Y-Axis (3 lines: Top, Mid, Bot)
			[0, 0.5, 1].forEach(tick => {
				const y = paddingTop + (drawHeight * (1 - tick));
				const val = Math.round(minVal + (range * tick));
			...
			// X-Axis (Time Labels - 3 or 4 points)
			const labels = ['-24H', '-12H', '-6H', 'NOW'];
			labels.forEach((lbl, i) => {
				const x = paddingLeft + (drawWidth * (i / (labels.length - 1)));

				// Grid tick
				ctx.beginPath();
				ctx.moveTo(x, paddingTop + drawHeight);
				ctx.lineTo(x, paddingTop + drawHeight + 5);
				ctx.stroke();
			...
			const points = data.map((val, i) => ({
				x: paddingLeft + (i * drawWidth) / (data.length - 1),
				y: paddingTop + drawHeight * (1 - (val - minVal) / range)
			}));

			// 2. Area Fill
			const gradient = ctx.createLinearGradient(0, paddingTop, 0, height - paddingBottom);
			gradient.addColorStop(0, lineColor.replace('1)', '0.3)').replace(')', ', 0.3)'));
		...
		updateAllDisplays() {
			...
			this.updateElement('lux-value', `${this.values.lux} LX`);
			...
			const lightBtn = document.getElementById('light-toggle');
			if (lightBtn) {
				lightBtn.classList.toggle('active', this.values.lightsOn);
				const status = document.getElementById('light-status');
				const orb = document.getElementById('light-orb');
				if (status) status.textContent = this.values.lightsOn ? 'ON' : 'OFF';
				if (orb) orb.classList.toggle('active', this.values.lightsOn);
			}
		}
	...
	async toggleLightBtn() {
			this.values.lightsOn = !this.values.lightsOn;
			this.updateAllDisplays();
			this.toggleLights(this.values.lightsOn);
		}
	...
	async toggleNutrient(type, on) {
		...
		connectWebSocket() {
			...
			this.ws.onopen = () => {
				this.addLogEntry('Telemetry Link: ESTABLISHED');
				...
				if (statusText) { statusText.textContent = 'ONLINE'; statusText.className = 'status-text success'; }
			};

			this.ws.onmessage = (event) => {
				try {
					const packet = JSON.parse(event.data);
					if (packet.type === 'GREENHOUSE_TELEMETRY') {
						this.handleTelemetry(packet.data);
					}
				} catch (e) {
			...
			this.ws.onclose = () => {
				this.addLogEntry('Telemetry Link: OFFLINE (Retrying...)');
				const statusDot = document.getElementById('mqtt-status-dot');
				const statusText = document.getElementById('mqtt-status-text');
				if (statusDot) { statusDot.className = 'status-indicator offline'; }
				if (statusText) { statusText.textContent = 'OFFLINE'; statusText.className = 'status-text warning'; }
		...
		handleTelemetry(data) {
			...
			// Update values with safety fallbacks to prevent NaN
			this.values.lux = Math.round(data.lux || this.values.lux || 0);
			this.values.humidity = Math.round(data.humidity || this.values.humidity || 0);
			this.values.tempIn = Math.round(data.temperature || this.values.tempIn || 0);
			this.values.tempOut = Math.round(data.outside_temp || this.values.tempOut || 27);

			// Map Lux to the Target Bar (0-20000 Lux range)
			const luxPct = Math.min(100, Math.max(0, (this.values.lux / 20000) * 100));
			const targetFill = document.getElementById('lux-target-fill');
			if (targetFill) targetFill.style.width = `${luxPct}%`;
