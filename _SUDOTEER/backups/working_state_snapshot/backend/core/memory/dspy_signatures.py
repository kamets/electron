...
class AnalyzeNutrients(dspy.Signature):
	"""Analyze nutrient levels (NPK) and recommend feeding adjustments."""

	current_levels: str = dspy.InputField(desc="Current N-P-K levels and EC/pH")
	plant_type: str = dspy.InputField(desc="Type of plant")
	growth_stage: str = dspy.InputField(desc="Current growth stage (vegetative, flowering, etc.)")

	analysis: str = dspy.OutputField(desc="Analysis of nutrient status")
	recommended_mix: str = dspy.OutputField(desc="Recommended N-P-K ratio or nutrient solution adjustment")
	warnings: list[str] = dspy.OutputField(desc="Nutrient deficiency or toxicity warnings")

class ManageCropCycle(dspy.Signature):
	"""Analyze overall crop cycle progress and schedule key events."""

	crop_data: str = dspy.InputField(desc="Historical and current logs for the crop")
	environmental_history: str = dspy.InputField(desc="Summary of environmental conditions")

	current_phase: str = dspy.OutputField(desc="Current verified growth phase")
	next_milestone: str = dspy.OutputField(desc="Next major event (e.g., 'Flip to Bloom', 'Harvest')")
	estimated_harvest_date: str = dspy.OutputField(desc="Predicted harvest date")
	health_score: float = dspy.OutputField(desc="Calculated plant health score (0-1)")
...
class CalibrationCheck(dspy.Signature):
	"""Cross-references the agent's reasoning against the Context Sandwich to detect hallucinations."""

	query: str = dspy.InputField(desc="The user's original query")
	context: str = dspy.InputField(desc="The provided Context Sandwich (Facts, Procedures, Episodes)")
	reasoning_path: str = dspy.InputField(desc="The agent's proposed reasoning or plan")

	confidence_score: float = dspy.OutputField(desc="0.0 to 1.0 score of factual alignment")
	hallucination_detected: bool = dspy.OutputField(desc="True if the reasoning contradicts the context")
	correction_logic: str = dspy.OutputField(desc="Specific instructions to fix the reasoning if a hallucination is detected")
	reasoning: str = dspy.OutputField(desc="Logical analysis of the alignment")
